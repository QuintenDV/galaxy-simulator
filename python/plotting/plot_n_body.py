# This code was written 4 years ago for a project at university.
# I threw this together quickly and I haven't refactored any of this.
# I will clean this mess up later.
import numpy as np
...
for filenumber in range(len(files)):
    ...
    def animate(i, data):
        if (i < intro_frames):
            ...
            azim = 60*(math.sin(math.pi/2 * (i+1)/intro_frames))**6
            altazim = 30+180*(math.cos(math.pi/2 * (i+1)/intro_frames))
            ax1.view_init(azim, altazim)

            outzoom = view_width*(0.05 + 0.45*math.sin((i*math.pi/2)/intro_frames)**6)
            ...
            altazim = 30*(1 + 3*math.sin((i/outro_frames)*math.pi/2)**3)
            ax1.view_init(azim, altazim)
    ...
    print("Saving video...")
    anim.save(output_video, fps=22, extra_args=['-vcodec', 'libx264'], savefig_kwargs={'facecolor':plot_color}, dpi = fig.dpi)
    # anim.save("animation.gif", fps=22, writer="imagemagick",
    #         savefig_kwargs={'facecolor':plot_color}, dpi = fig.dpi)
    print("Done")
